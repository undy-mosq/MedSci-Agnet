...
| ------------------- | --------------------------------------------------------------------------------------------------------------------- |
| `__init__.py`       | 包标识                                                                                                                   |
| `routes_analyze.py` | `POST /api/analyze`（PubMed + 期刊指标 JOIN）；`POST /api/review`（综述）；`GET /api/journal-metrics`；`GET /api/ping` |
...
| 方法     | 路径                     | 说明                                                                                                                                                                                |
| ------ | ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST` | `/api/analyze`         | 检索 PubMed 并 JOIN 期刊指标，返回 `articles`、`total_hits`；语料统计、Top100、词云由前端计算。**不**调用 LLM；`review` 固定为 `null`。PubMed 失败时 HTTP 502。                                   |
...
- 综述与检索解耦：两次 HTTP 往返；统计/词云在浏览器侧由 `corpusStats` / `wordFreq` 计算。

## 修改记录

- 初版：创建分析路由与依赖注入。
- 分阶段综述：`/api/analyze` 不再生成 `review`；新增 `POST /api/review`。
- [2026-05-18] `/api/analyze` 瘦身：移除 stats、wordcloud、top100_if_5y。
